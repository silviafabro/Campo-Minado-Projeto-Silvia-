...
class TabuleiroTest {
    ...
    @Test
    void revelarTodasAsMinasExibeTambemAMinaQueEstavaMarcada() {
        ...
        for (int l = 0; l < tabuleiro.getLinhas(); l++) {
            for (int c = 0; c < tabuleiro.getColunas(); c++) {
                if (tabuleiro.isMinada(l, c)) {
                    ...
                    assertTrue(tabuleiro.isRevelada(l, c));
                    assertFalse(tabuleiro.isMarcada(l, c));
                    return;
                }
            }
        }

        fail("O tabuleiro deveria conter uma mina");
    }
}
